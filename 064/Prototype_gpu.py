...
if len(sys.argv)>1:
    ...
    analysis_name= '' #'D:/SurgeryAnalytics/BoneAgeCode/BoneAge/valid/F160/16y-F-239.png' #目前已不使用
...
def predict( model, test_image_name):
    ...
    with torch.no_grad():
        ...
        # net.load_state_dict(torch.load(PATH)) # ref from net
...
model = torch.load(best_epoch)    #1120804  old

# MODEL_PATH = 'Somemodelname.pth'
model = model.load_state_dict(torch.load(best_epoch, 
