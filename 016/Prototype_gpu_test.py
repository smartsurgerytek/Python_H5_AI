...
class Demo():
    ...
    def check_transfer_file_time(self): #1110928
        ...
        t = self.t2-self.t1
        print(str(date.today()) + "##" + str(self.t1) + "##" + str(self.t2) + "##" + str(round(t.total_seconds()*1000))+"毫秒")        
        with open(output, 'a', newline="") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerow([date.today(), self.t1, self.t2, round(t.total_seconds()*1000),'毫秒'])
