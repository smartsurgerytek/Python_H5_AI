...
class PredFolderImg(object):
    ...
    def createModel(self):
        print(self.backboneName,self.modelName)
    ...
    def get_pred_img(self, img_path): #1110911
        ...
        print(self.pred_label.shape[0],self.pred_label.shape[1])
        print(self.width,self.hight)
        pred_img = np.zeros([self.width, self.hight, 3]) #分析及了解中....1120731
        unique_label_list = np.unique(self.pred_label).tolist()
        print("異常的位置") #1120720
        print(self.pred_label)
        values = np.unique(self.pred_label).tolist()
        print(values)
        # output: [1 2 3 4 6]
        # print(counts)
        # output: [1 3 1 1 1]
        print(len(unique_label_list)) #1120720
        if len(unique_label_list) >= 1:
            try:
                unique_label_list.remove(0) # 移除值為2的元素
                print("移除值為0的元素 Success")
            except IOError: # 捕捉輸出入錯誤
                print("移除值為0的元素 Error")
            except:
                print("移除值為0的元素 Error")
        if len(unique_label_list) >= 1:
            try:
                unique_label_list.remove(3) # 移除值為2的元素
                print("移除值為3的元素 Success")
            except IOError: # 捕捉輸出入錯誤
                print("移除值為3的元素 Error")
            except:
                print("移除值為3的元素 Error")
    ...
    def get_pred_label(self, img): #1110911
        resize_img = cv2.resize(img, (self.data_width, self.data_height), interpolation=cv2.INTER_CUBIC) 
        resize_img = self.preprocessInput(resize_img)
    ...
    def get_label_text_img(self):
        ...
        print(len(unique_list))  #1120721 unique_list charley       
        if len(unique_list) >= 1:
            try:
                unique_list.remove(0) # 移除值為2的元素 1120726
            except IOError: # 捕捉輸出入錯誤
                print("移除值為0的元素 Error")
            except:
                print("移除值為0的元素 Error")
        if len(unique_list) >= 1:
            try:
                unique_list.remove(3) # 移除值為2的元素 1120726
            except IOError: # 捕捉輸出入錯誤
                print("移除值為0的元素 Error")
            except:
                print("移除值為0的元素 Error")
    ...
    def run(self):        
        ...
        for ckptFileName in os.listdir(self.ckptFolderPath):
            ...
            print(ckptPath)
            self.loadModelWeight(ckptPath)# had erreo 1120713
            ...
            try:
                 while True:
                    ...
                    sys.stdout.write('\r')
                    sys.stdout.write("Wait for new surgical pictures " + "."*self.dot_count + " "*8)
